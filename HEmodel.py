...
class crossflow_PCHE(object):
    ...
    def __init__(self, reactant, utility, fuel, dimensions):
        ...
        #solid phase properties， 304 stainless steel
        self.metalrho = 8000
        self.metalcp = 500 #[J/kgC]
        self.metalk = 15 #Thermal conductivity of 15 W/m to represent stainless steel
...
dimensions = [0.003, 0.003, 5, 5, 0.0012, 0.001]
...
t0 = time.time()
solution = solve_ivp(exchanger.transient_solver, [t_span[0], t_span[0+1]], initial_temps, method = 'BDF', t_eval = [t_span[0], t_span[0+1]])
exchanger.update_pressures()

#Adding pressure
P_process2 = np.append(P_process2, exchanger.reactant2_P.min())
P_utility1 = np.append(P_utility1, exchanger.utility1_P.min())
P_fuel3 = np.append(P_fuel3, exchanger.fuel3_P.min())
P_process4 = np.append(P_process4, exchanger.reactant4_P.min())
P_utility5 = np.append(P_utility5, exchanger.utility5_P.min())

#Adding temperature
T_U1, T_P2, T_F3, T_P4, T_U5, T_U_P1, T_P_P2, T_F_P3, T_P_P4, T_U_P5 = convert_T_vector(solution['y'][:, -1], dimensions)
T_process2 = np.append(T_process2,T_P2)
T_utility1 = np.append(T_utility1,T_U1)
T_fuel3 = np.append(T_fuel3,T_F3)
T_process4 = np.append(T_process4,T_P4)
T_utility5 = np.append(T_utility5,T_U5)
T_process_plate2 = np.append(T_process_plate2,T_P_P2)
T_utility_plate1 = np.append(T_utility_plate1,T_U_P1)
T_fuel_plate3 = np.append(T_fuel_plate3,T_F_P3)
T_process_plate4 = np.append(T_process_plate4,T_P_P4)
T_utility_plate5 = np.append(T_utility_plate5,T_U_P5)

for i in range(1,len(t_span)-1):
    solution = solve_ivp(exchanger.transient_solver, [t_span[i], t_span[i+1]], solution['y'][:, -1], method = 'BDF', t_eval = [t_span[i], t_span[i+1]])
    exchanger.update_pressures()
...
for i in range (0,len(t_span)-1):
    ...
    print("Data at time: " + str(t_span[i+1]) + ' s')
    print('T_process 2')
    print(str(np.reshape(T_process2[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_utility1 ')
    print(str(np.reshape(T_utility1[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_fuel 3')
    print(str(np.reshape(T_fuel3[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_process 4')
    print(str(np.reshape(T_process4[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_utility 5')
    print(str(np.reshape(T_utility5[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_process_plate 2')
    print(str(np.reshape(T_process_plate2[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_utility_plate 1')
    print(str(np.reshape(T_utility_plate1[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_fuel_plate 3')
    print(str(np.reshape(T_fuel_plate3[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_process_plate 4')
    print(str(np.reshape(T_process_plate4[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('T_utility_plate 5')
    print(str(np.reshape(T_utility_plate5[i*dimensions[2]**2:(i+1)*dimensions[2]**2],(dimensions[2],dimensions[2]))).replace(' [', '').replace('[', '').replace(']', ''))
    print()
    print('P_process 2: ' + str(P_process2[i]))
    print('P_utility 1: ' + str(P_utility1[i]))
    print('P_fuel 3: ' + str(P_fuel3[i]))
    print('P_process 4: ' + str(P_process4[i]))
    print('P_utility 5: ' + str(P_utility5[i]))
    print()
    ...
    m1 = ax1.matshow(np.reshape(np.round(T_process2[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax1.set_title('T_process2')
    divider = make_axes_locatable(ax1)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m1, cax=cax, orientation='horizontal')

    ax2 = fig.add_subplot(2,5,2)
    m2 = ax2.matshow(np.reshape(np.round(T_utility1[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax2.set_title('T_utility1')
    divider = make_axes_locatable(ax2)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m2, cax=cax, orientation='horizontal')
    
    ax3 = fig.add_subplot(2,5,3)
    m3 = ax3.matshow(np.reshape(np.round(T_fuel3[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax3.set_title('T_fuel3')
    divider = make_axes_locatable(ax3)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m3, cax=cax, orientation='horizontal')

    ax4 = fig.add_subplot(2,5,4)
    m4 = ax4.matshow(np.reshape(np.round(T_process4[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax4.set_title('T_process4')
    divider = make_axes_locatable(ax4)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m4, cax=cax, orientation='horizontal')
    
    ax5 = fig.add_subplot(2,5,5)
    m5 = ax5.matshow(np.reshape(np.round(T_utility5[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax5.set_title('T_utility5')
    divider = make_axes_locatable(ax5)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m5, cax=cax, orientation='horizontal')
    
    ax6 = fig.add_subplot(2,5,6)
    m6 = ax6.matshow(np.reshape(np.round(T_process_plate2[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax6.set_title('T_process_plate2')
    divider = make_axes_locatable(ax6)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m6, cax=cax, orientation='horizontal')

    ax7 = fig.add_subplot(2,5,7)
    m7 = ax7.matshow(np.reshape(np.round(T_utility_plate1[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax7.set_title('T_utility_plate1')
    divider = make_axes_locatable(ax7)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m7, cax=cax, orientation='horizontal')
    
    ax8 = fig.add_subplot(2,5,8)
    m8 = ax8.matshow(np.reshape(np.round(T_fuel_plate3[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax8.set_title('T_fuel_plate3')
    divider = make_axes_locatable(ax8)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m8, cax=cax, orientation='horizontal')

    ax9 = fig.add_subplot(2,5,9)
    m9 = ax9.matshow(np.reshape(np.round(T_process_plate4[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax9.set_title('T_process_plate4')
    divider = make_axes_locatable(ax9)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m9, cax=cax, orientation='horizontal')
    
    ax10 = fig.add_subplot(2,5,10)
    m10 = ax10.matshow(np.reshape(np.round(T_utility_plate5[i*dimensions[2]**2:(i+1)*dimensions[2]**2],1),(dimensions[2],dimensions[2])),interpolation = "nearest")
    ax10.set_title('T_utility_plate5')
    divider = make_axes_locatable(ax10)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    fig.colorbar(m10, cax=cax, orientation='horizontal')
